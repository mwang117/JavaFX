...
public class CarteVisite extends Application implements EventHandler<MouseEvent> {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		//File imageFile = new File("D:\\BUT1\\R2_01\\JAVAFX\\TD\\src\\TD3\\tazz.png");
		//Image icon = new Image(imageFile.toURI().toURL().toString());
		//primaryStage.getIcons().add(icon);
	}
	...
	private Group dessiner() {
	    ...
	    File imageFile = new File("tazz.png");
	    Image img = null;
	    try {
	        img = new Image(imageFile.toURI().toURL().toString(), 140, 140, true, false);
	    } catch (Exception e) {
	        e.printStackTrace();
	    }
	...
	public void handle(MouseEvent event) {
		if (event.getSource() == this.btnCréerCarte) {
			...
			// new FileChooser.ExtensionFilter("Fichiers images", "*.png",
			// "*.jpg", "*.jpeg", "*.gif")
			// );
